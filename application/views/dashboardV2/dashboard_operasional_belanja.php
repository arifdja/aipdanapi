...
<script type="text/javascript">
    ...
    $.post(host+'dashboard-tampil/get_operasional_belanja', par, function(resp){
        if(resp){
    ...
    $('.filter_data').on('click', function(){
        ...
        if ($('#dashboard').val() == "BULANAN") {
            $.messager.alert('SMART AIP','Data laporan bulanan tidak tersedia !','warning');
            return false;
        }
        ...
        $.LoadingOverlay("show");
        $.post(host+'dashboard-tampil/get_operasional_belanja', param, function(resp){
            if(resp){
                $.LoadingOverlay("hide", true);
