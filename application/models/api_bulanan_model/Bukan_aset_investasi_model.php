...
class Bukan_aset_investasi_model extends CI_Model {
  ...
  public function get($bulan=null,$tahun=null,$user=null)
  {
    ...
    $id = $this->db->get_where('mst_investasi',array('mst_investasi.group'=>'BUKAN INVESTASI','mst_investasi.iduser'=>$user))->result_array();
  ...
  public function delete($bulan=null,$tahun=null,$user=null)
  {
    ...
    if($status_input == true){
      ...
      //Cari id tabel detil untuk BUKAN INVESTASI
      $this->db->select('id');
      $this->db->where('id_bulan',$bulan);
      $this->db->where('tahun',$tahun);
      $this->db->where('iduser',$user);
   ...
   */
  private function validasi_bukan_investasi($id_investasi,$key,$data,$detail)
  {
    ...
    foreach($detail as $keyDet => $v){
      ...
      if ($v->saldo_awal + $v->mutasi_pembelian - $v->mutasi_penjualan != $v->saldo_akhir) {
        $msg.= '<< Detil id_investasi '.$id_investasi.' kode_pihak '.$v->kode_pihak.' tidak valid >>';
      }

    }

    $sum_mutasi = $sum_mutasi_pembelian - $sum_mutasi_penjualan;
  ...
  public function insert($data,$tkn)
  {
    ...
    $this->db->select('id_investasi');
    $id = $this->db->get_where('mst_investasi',array('mst_investasi.group'=>'BUKAN INVESTASI','mst_investasi.iduser'=>$tknid))->result_array();
    $arrID = array();
    foreach ($id as $key => $value) {
      $arrID[] = $value['id_investasi'];
    }
    ...
    foreach ($data as $key => $value) {
      ...
      if (in_array($id_investasi, $arrID) && in_array($id_user, $arrUSER) && in_array($id_bulan, $arrBulan)) {
        ...
        if($status_input == true)
        {
          ...
          // var_dump($invalid_id_investasi);exit;
            ...
            if ($cekdataDetail>0) {
              ...
              foreach($detail as $keyDet => $v){
                ...
                $dataInsertDetail = array(
                  ...
                  'mutasi_penanaman' => escape($v->mutasi_pencairan),
