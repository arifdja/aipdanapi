...
class Nilai_investasi_model extends CI_Model {
  ...
  public function get($bulan=null,$tahun=null,$user=null)
  {
    ...
    $id = $this->db->get_where('mst_investasi',array('mst_investasi.group'=>'NILAI INVESTASI','mst_investasi.iduser'=>$user))->result_array();
  ...
  public function insert($data,$tkn)
  {
    ...
    $this->db->select('id_investasi');
    $id = $this->db->get_where('mst_investasi',array('mst_investasi.group'=>'NILAI INVESTASI','mst_investasi.iduser'=>$tknid))->result_array();
    $arrID = array();
    foreach ($id as $key => $value) {
      $arrID[] = $value['id_investasi'];
    }
