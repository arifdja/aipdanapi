...
class Kewajiban_model extends CI_Model {
  ...
  public function get($bulan=null,$tahun=null,$user=null)
  {
    ...
    $id = $this->db->get_where('mst_investasi',array('mst_investasi.group'=>'KEWAJIBAN','mst_investasi.iduser'=>$user))->result_array();
  ...
  public function insert($data,$tkn)
  {
    ...
    $this->db->select('id_investasi');
    $id = $this->db->get_where('mst_investasi',array('mst_investasi.group'=>'KEWAJIBAN','mst_investasi.iduser'=>$tknid))->result_array();
    $arrID = array();
    foreach ($id as $key => $value) {
      $arrID[] = $value['id_investasi'];
    }
    ...
    foreach ($data as $key => $value) {
      ...
      if (in_array($id_investasi, $arrID) && in_array($id_user, $arrUSER) && in_array($id_bulan, $arrBulan)) {
        // jika key nya not null maka id investasi merupakan KEWAJIBAN
        ...
        $status = 0;
        // jika key nya null maka error karna bukan KEWAJIBAN
      }
    }
