...
class Ikhtisar_kinerja_model extends CI_Model {
	private $table;
	...
	public function get_all($limit_offset = array()){
		if(!empty($limit_offset)){
			...
			$query = $this->db->get('ket_lap_bulanan',$limit_offset['limit'],$limit_offset['offset']);
		}else{
			$query = $this->db->get('ket_lap_bulanan');
		}
		return $query->result();
	}
	...
	public function get_all_array($filter = false){
		if(!empty($filter)) {
			$query = $this->db->get_where('ket_lap_bulanan', $filter);
		}else{
			$query = $this->db->get('ket_lap_bulanan');
		}
	...
	public function get_last_id(){
		$this->db->order_by('id', 'DESC');
		$query = $this->db->get('ket_lap_bulanan',1,0);
		return $query->result();
	}
	...
	public function update($bulan,$iduser,$data){
		...
		$this->db->update('ket_lap_bulanan', $data);
	}

	public function get_by_id($id){
		$response = false;
		$query = $this->db->get_where('ket_lap_bulanan',array('id' => $id,'iduser' => $this->iduser));
	...
	public function delete($data){
		...
		$this->db->delete('ket_lap_bulanan', array('id_bulan' => $bulan,'iduser' => $this->iduser));
	}

	
	public function get_filter($filter = '',$limit_offset = array()){
		if(!empty($filter)){
			$query = $this->db->order_by("id_bulan", "desc")->get_where('ket_lap_bulanan',$filter);
		}else{
			$query = $this->db->get('ket_lap_bulanan',$limit_offset['limit'],$limit_offset['offset']);
		}
	...
	public function get_by_id_ket($id){
		$response = false;
		$query = $this->db->get_where('ket_lap_bulanan',array('id' => $id,'iduser' => $this->iduser,'iduser' => $this->iduser));
		if($query && $query->num_rows()){
			$response = $query->result_array();
		}
		return $response;
	...
	public function delete_ket($p1){
		$this->db->delete('ket_lap_bulanan', array('id_bulan' => $p1, 'iduser' => $this->iduser,'iduser' => $this->iduser));
	}
